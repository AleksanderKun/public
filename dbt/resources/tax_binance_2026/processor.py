...
class DataProcessor:
    ...
    def normalize_and_classify(self) -> tuple[int, int]:
        ...
        if error_count > 0:
            ...
            for error in self.normalizer.get_errors():
                logger.warning(f"  Row {error.row_index}: {error.error_type} - {error.message}")
    ...
    def apply_currency_conversion(self) -> int:
        ...
        logger.info(f"Applying currency conversion for {len(self.normalized_transactions)} transactions...")
        converted_count = 0
    ...
    def calculate_tax(self) -> TaxReport:
        ...
        """
        if not self.normalized_transactions:
            raise RuntimeError("No transactions to calculate. Call normalize_and_classify() first.")
