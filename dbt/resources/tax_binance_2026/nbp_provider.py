...
class NBPRateProvider:
        ...
        if self.cache_path and self.cache_path.exists():
            try:
                with open(self.cache_path, 'r', encoding='utf-8') as f:
                    ...
                    self.cache.update({
                        k: Decimal(str(v)) for k, v in cached.items()
                    })
                logger.debug(f"Loaded cache from {self.cache_path} ({len(self.cache)} entries)")
    ...
    def _fetch_rate_for_date(
        ...
        try:
            response = requests.get(url, timeout=self.timeout, params={"format": "json"})
            ...
            logger.warning(f"Failed to parse rate data for {currency} on {date_str}: {e}")
            return None
