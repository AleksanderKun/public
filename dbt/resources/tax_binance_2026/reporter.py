...
class ReportGenerator:
    ...
    def generate_text_summary(
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Data raportu: {report.report_date.strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
            ...
            f.write("-" * 70 + "\n")
            f.write(
                f"Całkowity przychód:        {summary['total_revenue_pln']:>15,.2f} PLN\n"
            )
            f.write(
                f"Całkowity koszt:          {summary['total_costs_pln']:>15,.2f} PLN\n"
            )
            f.write(
                f"Całkowite opłaty:         {summary['total_fees_pln']:>15,.2f} PLN\n"
            )
            f.write(
                f"Całkowity dochód podatkowy: {summary['total_taxable_income']:>13,.2f} PLN\n"
            )
            f.write(
                f"Całkowity podatek (19%):  {summary['total_tax_due']:>15,.2f} PLN\n\n"
            )
            ...
            for year in sorted(report.years.keys()):
                ...
                f.write(
                    f"  Przychód:                {ty.total_revenue_pln:>15,.2f} PLN\n"
                )
                f.write(
                    f"  Koszt:                   {ty.total_costs_pln:>15,.2f} PLN\n"
                )
                f.write(f"  Opłaty:                  {ty.total_fees_pln:>15,.2f} PLN\n")
                f.write(
                    f"  Razem koszt:             {ty.total_cost_with_fees:>15,.2f} PLN\n"
                )
                ...
                if ty.loss_from_previous_years > 0:
                    f.write(
                        f"  Strata z lat poprzednich: {ty.loss_from_previous_years:>15,.2f} PLN\n"
                    )

                f.write(f"  Dochód do opodatkowania: {ty.taxable_income:>15,.2f} PLN\n")
                f.write(
                    f"  Podatek (19%):           {ty.tax_due_19_percent:>15,.2f} PLN\n"
                )
