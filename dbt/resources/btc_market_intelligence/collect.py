...
async def main(provider: str) -> None:
    ...
    store = (
        ...
        else DuckDBStore(database)
    )
    collector = PublicMarketCollector()
    jobs = {
        "binance": collector.collect_binance,
        "bybit": collector.collect_bybit,
        "okx": collector.collect_okx,
        "binance_ohlcv": collector.collect_binance_ohlcv,
    }
    ...
    try:
        for name, job in selected.items():
            try:
                ...
                print(
                    f"{name}: received={len(observations)} inserted={store.write(observations)}"
                )
