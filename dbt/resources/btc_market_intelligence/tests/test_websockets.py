...
def test_binance_aggregate_trade_has_provenance_and_two_metrics():
    observations = parse_binance_message({"e": "aggTrade", "E": 1700000000000, "s": "BTCUSDT", "a": 7, "p": "42000.1", "q": "0.25", "m": True})
...
def test_binance_combined_stream_is_unwrapped():
    observations = parse_binance_message({"stream": "btcusdt@bookTicker", "data": {"e": "bookTicker", "E": 1700000000000, "s": "BTCUSDT", "b": "42000", "B": "1", "a": "42001", "A": "2"}})
    assert [item.metric for item in observations] == ["best_bid_price", "best_ask_price"]


def test_bybit_liquidation_is_not_claimed_as_private_liquidation_price():
    observations = parse_bybit_message({"topic": "allLiquidation.BTCUSDT", "ts": 1700000000000, "data": [{"s": "BTCUSDT", "S": "Sell", "p": "42000", "v": "0.5", "T": 1700000000000}]})
...
def test_okx_trade_and_delta_parsers_preserve_exchange_fields():
    ...
    trade = adapter.parse_message({"arg": {"channel": "trades"}, "data": [{"instId": "BTC-USDT-SWAP", "tradeId": "9", "px": "42000", "sz": "2", "side": "buy", "ts": "1700000000000"}]})[0]
    delta = adapter.parse_message({"arg": {"channel": "books5"}, "data": [{"instId": "BTC-USDT-SWAP", "bids": [["42000", "2", "0", "1"]], "asks": [["42001", "3", "0", "1"]], "seqId": 11, "prevSeqId": 10, "ts": "1700000000000"}]})[0]
...
def test_binance_synchronizer_discards_stale_and_bridges_buffered_updates():
    ...
    synchronizer.buffer_update({"U": 95, "u": 99, "b": [["100", "2"]], "a": [["101", "2"]]})
    synchronizer.buffer_update({"U": 100, "u": 101, "b": [["100", "3"]], "a": []})
    synchronizer.buffer_update({"U": 102, "u": 103, "b": [], "a": [["101", "1"]]})
    assert synchronizer.synchronize([["100", "1"]], [["101", "1"]], last_update_id=99) is True
...
def test_binance_futures_requires_pu_to_match_previous_u():
    ...
    synchronizer = BinanceOrderBookSynchronizer()
    synchronizer.synchronize([["100", "1"]], [["101", "1"]], last_update_id=100)
    assert synchronizer.apply_update({"U": 101, "u": 102, "pu": 999, "b": [], "a": []}) is False
...
def test_binance_futures_accepts_update_range_when_pu_matches_previous_u():
    ...
    synchronizer = BinanceOrderBookSynchronizer()
    synchronizer.synchronize([["100", "1"]], [["101", "1"]], last_update_id=100)
    assert synchronizer.apply_update({"U": 150, "u": 160, "pu": 100, "b": [], "a": []}) is True
...
def test_bounded_batch_writer_applies_backpressure_and_flushes():
    ...
    async def exercise():
        writer = BoundedBatchWriter(write_batch, max_queue_size=2, batch_size=2, flush_interval_ms=10)
        await writer.start()
        event = WebSocketEvent("okx", "trades", "BTCUSDT", "perpetual", datetime.now(timezone.utc), trade_id="1", price=100, quantity=1)
...
def test_bounded_batch_writer_does_not_deadlock_after_batch_error():
    ...
    async def exercise():
        writer = BoundedBatchWriter(failing_batch, max_queue_size=2, batch_size=2, flush_interval_ms=10)
        await writer.start()
        event = WebSocketEvent("binance", "trades", "BTCUSDT", "perpetual", datetime.now(timezone.utc), trade_id="1", price=100, quantity=1)
        await writer.put(event)
        await writer.stop()
        return writer
...
def test_websocket_delta_and_snapshot_storage_are_separate(tmp_path):
    ...
    event = WebSocketEvent("okx", "orderbook", "BTCUSDT", "perpetual", datetime.now(timezone.utc), bids=((100.0, 2.0),), asks=((101.0, 3.0),), sequence_id=11, update_type="delta", raw_payload={"seqId": 11})
    snapshot = WebSocketEvent("okx", "orderbook", "BTCUSDT", "perpetual", event.event_timestamp, bids=((100.0, 2.0),), asks=((101.0, 3.0),), sequence_id=11, update_type="snapshot", raw_payload={"seqId": 11})
    ...
    assert store.connection.execute("SELECT COUNT(*) FROM raw_orderbook_updates").fetchone()[0] == 1
    assert store.connection.execute("SELECT COUNT(*) FROM raw_orderbook").fetchone()[0] == 2
    store.close()
