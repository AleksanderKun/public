...
from btc_intelligence.analytics import classify_market_regime, compute_cvd, explainable_score, market_report, market_structure, ohlcv_indicators, scenario_estimates
...
def test_duckdb_storage_is_idempotent(tmp_path):
    ...
    observation = Observation(datetime(2026, 8, 22, tzinfo=timezone.utc), "fixture", "binance", "spot", "BTCUSDT", "price", 100.0)
...
def test_indicators_and_report_separate_facts_from_models():
    candles = [{"open": 100 + i, "high": 102 + i, "low": 99 + i, "close": 101 + i} for i in range(16)]
    indicators = ohlcv_indicators(candles)
    regime = classify_market_regime({"price_change_pct": 2, "oi_change_pct": 0.5, "funding_rate": 0.0001, "spot_cvd_change": 2})
    report = market_report(["provider observation exists"], regime, ["continuation remains possible"])
...
def test_raw_schema_and_trade_normalization_include_exchange_specific_metadata():
    ...
    assert store.connection.execute("SELECT COUNT(*) FROM raw_trades").fetchone()[0] == 2
    assert store.connection.execute("SELECT COUNT(*) FROM raw_orderbook").fetchone()[0] == 0
    store.close()
...
def test_exchange_market_data_adapters_match_common_contract():
    ...
    bybit_trade = bybit.normalize_trade({
        ...
        "S": "Buy",
        "T": 1700000001000,
    }, symbol="BTCUSDT", market_type="perpetual")
    ...
    assert binance.validate_event({"timestamp": "2026-08-22T12:00:00Z", "price": 10, "quantity": 0.5})["status"] == "VALID"
...
def test_ingestion_service_persists_and_deduplicates_raw_events(tmp_path):
    ...
    class FixtureAdapter(BinanceMarketDataAdapter):
        async def get_trades(self, symbol="BTCUSDT", market_type="spot", limit=20):
            return [self.normalize_trade({"a": 1, "p": "100", "q": "2", "m": False, "T": 1700000000000}, symbol, market_type)] * 2
    ...
    assert store.connection.execute("SELECT COUNT(*) FROM raw_trades").fetchone()[0] == 1
    store.close()
...
def test_ingestion_service_rejects_crossed_orderbook_and_keeps_diagnostic(tmp_path):
    ...
    class FixtureAdapter(BinanceMarketDataAdapter):
        async def get_orderbook(self, symbol="BTCUSDT", market_type="perpetual", depth=20):
            return [self.normalize_orderbook({"bids": [["101", "1"]], "asks": [["100", "1"]]}, symbol, market_type)]
    ...
    assert store.connection.execute("SELECT COUNT(*) FROM raw_orderbook").fetchone()[0] == 0
    assert store.connection.execute("SELECT COUNT(*) FROM rejected_raw_events").fetchone()[0] == 1
    store.close()