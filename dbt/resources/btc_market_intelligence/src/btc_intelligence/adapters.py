...
class ExchangeMarketDataAdapter:
    ...
    def validate_event(self, event: dict[str, Any]) -> dict[str, Any]:
        timestamp = (
            ...
            or event.get("source_timestamp_utc")
        )
    ...
    async def _request_with_retry(
        ...
        raise RuntimeError(
            f"{self.exchange} request failed after retries: {last_error}"
        )

    async def get_trades(
        self, symbol: str = "BTCUSDT", market_type: str = "spot", limit: int = 20
    ) -> list[dict[str, Any]]:
        raise NotImplementedError

    async def get_orderbook(
        self, symbol: str = "BTCUSDT", market_type: str = "spot", depth: int = 20
    ) -> list[dict[str, Any]]:
        raise NotImplementedError

    async def get_open_interest(
        self, symbol: str = "BTCUSDT", market_type: str = "perpetual"
    ) -> list[dict[str, Any]]:
        raise NotImplementedError
    ...
    async def get_liquidations(
        self, symbol: str = "BTCUSDT", market_type: str = "perpetual", limit: int = 20
    ) -> list[dict[str, Any]]:
        return [
            {
                "status": "NOT_AVAILABLE",
                "reason": f"{self.exchange} does not expose a supported public liquidation endpoint in this adapter.",
            }
        ]

    def normalize_trade(
        self, payload: dict[str, Any], symbol: str, market_type: str
    ) -> dict[str, Any]:
        raise NotImplementedError
...
class BinanceMarketDataAdapter(ExchangeMarketDataAdapter):
    ...
    def normalize_trade(
        ...
        aggressive_side = (
            "sell"
            if payload.get("m", payload.get("isBuyerMaker")) is True
            else "buy"
            if payload.get("m", payload.get("isBuyerMaker")) is False
            else "unknown"
        )
        event_ms = payload.get("T") or payload.get("E") or payload.get("time")
        event = {
            "event_timestamp": datetime.fromtimestamp(
                int(event_ms or 0) / 1000, tz=timezone.utc
            ).isoformat(),
    ...
    def normalize_open_interest(
        ...
        return {
            "event_timestamp": datetime.fromtimestamp(
                int(payload.get("time") or 0) / 1000, tz=timezone.utc
            ).isoformat(),
            "exchange": self.exchange,
            "symbol": self.canonicalize_symbol(symbol),
            "market_type": market_type,
    ...
    def normalize_funding(
        ...
        return {
            "event_timestamp": datetime.fromtimestamp(
                int(payload.get("fundingTime") or payload.get("time") or 0) / 1000,
                tz=timezone.utc,
            ).isoformat(),
            "exchange": self.exchange,
            "symbol": self.canonicalize_symbol(symbol),
    ...
    def normalize_liquidation(
        ...
        return {
            "event_timestamp": datetime.fromtimestamp(
                int(payload.get("T") or 0) / 1000, tz=timezone.utc
            ).isoformat(),
            "exchange": self.exchange,
            "symbol": self.canonicalize_symbol(symbol),
            "market_type": market_type,
    ...
    def normalize_orderbook(
        ...
        best_bid_price = (
            _safe_float((bid[0][0] if isinstance(bid, list) and bid else None))
            if bid
            else None
        )
        best_ask_price = (
            _safe_float((ask[0][0] if isinstance(ask, list) and ask else None))
            if ask
            else None
        )
        bid_quantity = (
            _safe_float(
                (
                    bid[0][1]
                    if isinstance(bid, list) and bid and len(bid[0]) > 1
                    else None
                )
            )
            if bid
        ...
        ask_quantity = (
            _safe_float(
                (
                    ask[0][1]
                    if isinstance(ask, list) and ask and len(ask[0]) > 1
                    else None
                )
            )
            if ask
    ...
    async def get_trades(
        ...
        endpoint = (
            ...
            else "https://api.binance.com/api/v3/trades"
        )
        payload = await self._request_with_retry(
            endpoint, {"symbol": self.canonicalize_symbol(symbol), "limit": limit}
        )
        return [
            self.normalize_trade(item, symbol, market_type) for item in payload[:limit]
        ]
    ...
    async def get_orderbook(
        ...
        payload = await self._request_with_retry(
            ...
            {"symbol": self.canonicalize_symbol(symbol), "limit": min(depth, 100)},
        )
    ...
    async def get_funding(
        ...
        payload = await self._request_with_retry(
            ...
            {"symbol": self.canonicalize_symbol(symbol), "limit": 1},
        )
    ...
    async def get_liquidations(
        ...
        return [
            {
                "status": "NOT_AVAILABLE",
                "reason": "Binance public REST does not provide unauthenticated historical force-order events; use the public forceOrder WebSocket stream.",
            }
        ]
...
class BybitMarketDataAdapter(ExchangeMarketDataAdapter):
    ...
    def normalize_trade(
        ...
        event = {
            "event_timestamp": _to_utc(ts).isoformat()
            if _to_utc(ts)
            ...
            "source": self.source,
            "source_timestamp_utc": _to_utc(ts).isoformat()
            if _to_utc(ts)
            else utc_now().isoformat(),
            "ingestion_timestamp_utc": utc_now().isoformat(),
            "raw_payload_hash": _payload_hash(payload),
    ...
    def normalize_open_interest(
        ...
        return {
            "event_timestamp": _to_utc(timestamp).isoformat()
            if _to_utc(timestamp)
            else utc_now().isoformat(),
            "exchange": self.exchange,
            "symbol": self.canonicalize_symbol(symbol),
            "market_type": market_type,
    ...
    def normalize_funding(
        ...
        value = _safe_float(payload.get("fundingRate"))
        return {
            "event_timestamp": _to_utc(
                payload.get("fundingTime")
                or payload.get("timestamp")
                or payload.get("ts")
            ).isoformat()
            if _to_utc(
    ...
    def normalize_liquidation(
        ...
        return {
            "event_timestamp": _to_utc(
                payload.get("T") or payload.get("timestamp") or payload.get("ts")
            ).isoformat()
            if _to_utc(
                payload.get("T") or payload.get("timestamp") or payload.get("ts")
            )
    ...
    def normalize_orderbook(
        ...
        best_bid_price = _safe_float(
            best_bid[0] if isinstance(best_bid, list) and best_bid else None
        )
        best_ask_price = _safe_float(
            best_ask[0] if isinstance(best_ask, list) and best_ask else None
        )
        bid_quantity = _safe_float(
            best_bid[1] if isinstance(best_bid, list) and len(best_bid) > 1 else None
        )
        ask_quantity = _safe_float(
            best_ask[1] if isinstance(best_ask, list) and len(best_ask) > 1 else None
        )
        return {
            "timestamp": _to_utc(
                payload.get("ts") or payload.get("timestamp")
            ).isoformat()
            if _to_utc(payload.get("ts") or payload.get("timestamp"))
            else utc_now().isoformat(),
            "exchange": self.exchange,
            "symbol": self.canonicalize_symbol(symbol),
            "market_type": market_type,
    ...
    async def get_trades(
        ...
        return [
            self.normalize_trade(item, symbol, market_type) for item in rows[:limit]
        ]

    async def get_orderbook(
        self, symbol: str = "BTCUSDT", market_type: str = "perpetual", depth: int = 20
    ) -> list[dict[str, Any]]:
        payload = await self._request_with_retry(
    ...
    async def get_open_interest(
        ...
        return [
            self.normalize_open_interest(item, symbol, market_type) for item in rows[:1]
        ]
    ...
    async def get_liquidations(
        ...
        return [
            {
                "status": "NOT_AVAILABLE",
                "reason": "Bybit public REST does not expose a supported liquidation-history endpoint; use the public allLiquidation WebSocket stream.",
            }
        ]
...
class OKXMarketDataAdapter(ExchangeMarketDataAdapter):
    ...
    def normalize_trade(
        ...
        quantity = _safe_float(
            payload.get("sz") or payload.get("size") or payload.get("qty")
        )
    ...
    def normalize_funding(
        ...
        value = _safe_float(payload.get("fundingRate"))
        funding_time = _to_utc(
            payload.get("fundingTime") or payload.get("ts") or payload.get("timestamp")
        )
        return {
            "event_timestamp": _to_utc(
                payload.get("ts") or payload.get("timestamp")
    ...
    def normalize_liquidation(
        ...
        qty = _safe_float(
            payload.get("sz") or payload.get("qty") or payload.get("quantity")
        )
        price = _safe_float(payload.get("px") or payload.get("price"))
    ...
    async def get_trades(
        ...
        payload = await self._request_with_retry(
            ...
            {"instId": self._inst_id(symbol, market_type), "limit": limit},
        )
    ...
    async def get_orderbook(
        ...
        payload = await self._request_with_retry(
            ...
            {"instId": self._inst_id(symbol, market_type), "sz": min(depth, 25)},
        )
    ...
    async def get_open_interest(
        ...
        payload = await self._request_with_retry(
            ...
            {"instType": "SWAP", "instId": self._inst_id(symbol, market_type)},
        )
        rows = payload.get("data", []) if isinstance(payload, dict) else []
        return [
            self.normalize_open_interest(item, symbol, market_type) for item in rows[:1]
    ...
    async def get_funding(
        ...
        payload = await self._request_with_retry(
            ...
            {"instId": self._inst_id(symbol, market_type)},
        )
        rows = payload.get("data", []) if isinstance(payload, dict) else []
        return [self.normalize_funding(item, symbol, market_type) for item in rows[:1]]
    ...
    async def get_liquidations(
        ...
        if not rows:
            return [
                {
                    "status": "NOT_AVAILABLE",
                    "reason": "OKX liquidation endpoint returned no public rows.",
                }
            ]
        try:
            return [
                self.normalize_liquidation(item, symbol, market_type)
            ...
            return [
                {
                    "status": "NOT_AVAILABLE",
                    "reason": "OKX liquidation response did not expose a normal price and quantity event shape.",
                }
            ]
    ...
    def _inst_id(self, symbol: str, market_type: str) -> str:
        ...
        if market_type == "perpetual":
            return (
                f"{normalized[:-4]}-{normalized[-4:]}-SWAP"
                if normalized.endswith("USDT")
                else f"{normalized}-SWAP"
            )
