    ...
    if event_type == "aggTrade":
        ...
        return [
            _observation(
                ...
                float(payload["p"]),
                event_time,
                {"trade_id": payload["a"], "is_buyer_maker": payload["m"]},
            ),
            _observation(
                "binance",
                "spot",
    ...
    if event_type == "forceOrder":
        ...
        return [
            ...
            _observation(
                ...
                float(order["z"]),
                event_time,
                {"side": order["S"], "price": order["ap"], "order_status": order["X"]},
            ),
        ]
        ...
        if data.get("b"):
            observations.append(
                _observation(
                    ...
                    float(data["b"][0][0]),
                    event_time,
                    {"quantity": data["b"][0][1], "update_id": data.get("u")},
                )
            )
        if data.get("a"):
            observations.append(
                _observation(
                    ...
                    float(data["a"][0][0]),
                    event_time,
                    {"quantity": data["a"][0][1], "update_id": data.get("u")},
                )
            )
        return observations
    return []
...
async def consume_with_reconnect(
    ...
    parser: Callable[[dict[str, Any]], list[Observation]],
