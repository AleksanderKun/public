...
class WebSocketEvent:
    ...
    def raw_payload_hash(self) -> str:
        return hashlib.sha256(
            json.dumps(self.raw_payload, sort_keys=True, separators=(",", ":")).encode()
        ).hexdigest()
...
class LocalOrderBook:
    ...
    def _sequence_is_valid(
        ...
        if previous_sequence is not None:
            return (
                previous_sequence == self.last_sequence and sequence > previous_sequence
            )
        if self.sequence_mode == "binance":
            ...
            return (
                ...
                and first_sequence <= self.last_sequence + 1 <= sequence
            )
    ...
    def _is_sane(self) -> bool:
        return bool(
            ...
            and all(value >= 0 for value in self.bids.values())
            and all(value >= 0 for value in self.asks.values())
        )
    ...
    def _apply_levels(
        self, target: dict[float, float], levels: list[list[str | float]]
    ) -> None:
        for level in levels:
            if len(level) < 2:
                continue
...
class BinanceOrderBookSynchronizer:
    ...
    def synchronize(
        ...
        self.last_buffer_diagnostics = {
            ...
            "min_U": min(
                (int(update.get("U", -1)) for _, update in self.buffer), default=None
            ),
            "max_u": max(
                (int(update.get("u", -1)) for _, update in self.buffer), default=None
            ),
        ...
        buffered = [
            update
            for _, update in self.buffer
            if int(update.get("u", -1)) > last_update_id
        ]
        ...
        if not self.book.apply_delta(
            bridge.get("b", []),
            bridge.get("a", []),
            int(bridge["u"]),
            first_sequence=int(bridge["U"]),
        ):
            ...
            self.gap_diagnostics.append(
                {
                    ...
                    "current_u": final,
                    "expected": previous,
                    "received_pu": int(previous_update),
                    "event_timestamp": update.get("E"),
    ...
    def _discard_old_buffer(self) -> None:
        ...
        self.buffer = [
            (received, update) for received, update in self.buffer if received >= cutoff
        ]
...
class BinanceWebSocketAdapter(WebSocketMarketDataAdapter):
    ...
    def uri(self) -> str:
        ...
        stream = (
            ...
            else f"{self.symbol.lower()}@depth@100ms"
            if self.market_type == "perpetual"
            else f"{self.symbol.lower()}@depth20@100ms"
        )
    ...
    def parse_message(self, payload: dict[str, Any]) -> list[WebSocketEvent]:
        ...
        if event_type in {"aggTrade", "trade"}:
            ...
            return [
                WebSocketEvent(
                    ...
                    symbol,
                    self.market_type,
                    _timestamp(message.get("T") or message.get("E")),
                    str(message.get("a") or message.get("t")),
                    float(message["p"]),
                    float(message["q"]),
                    "sell" if maker is True else "buy" if maker is False else "unknown",
                    raw_payload=message,
                )
            ]
        if event_type in {"depthUpdate", "bookTicker"}:
            if event_type == "bookTicker":
                ...
                bids = tuple(
                    (float(row[0]), float(row[1])) for row in message.get("b", [])
                )
                asks = tuple(
                    (float(row[0]), float(row[1])) for row in message.get("a", [])
                )
            ...
            return [
                WebSocketEvent(
                    ...
                    first_sequence_id=message.get("U"),
                    previous_sequence_id=message.get("pu"),
                    update_type="delta",
                    raw_payload=message,
                )
            ]
        if event_type == "forceOrder":
            ...
            return [
                WebSocketEvent(
                    self.exchange,
                    "liquidations",
                    canonical_symbol(order.get("s", self.symbol)),
                    "perpetual",
                    event_ts,
                    price=price,
                    quantity=quantity,
        ...
        if topic.startswith("publicTrade."):
            return [
                WebSocketEvent(
                    self.exchange,
                    "trades",
                    canonical_symbol(row["s"]),
                    self.market_type,
                    _timestamp(row.get("T")),
                    str(row["i"]),
                    float(row["p"]),
                    float(row["v"]),
                    str(row.get("S", "unknown")).lower(),
                    raw_payload=row,
                )
                for row in data
            ]
        if topic.startswith("orderbook."):
            return [
                WebSocketEvent(
                    self.exchange,
                    "orderbook",
                    canonical_symbol(data["s"]),
                    self.market_type,
                    event_ts,
                    bids=tuple(
                        (float(row[0]), float(row[1])) for row in data.get("b", [])
                    ),
                    asks=tuple(
                        (float(row[0]), float(row[1])) for row in data.get("a", [])
                    ),
                    sequence_id=data.get("u"),
                    update_type=payload.get("type", "delta"),
                    raw_payload=payload,
                )
            ]
        if topic.startswith("allLiquidation."):
            ...
            return [
                WebSocketEvent(
                    self.exchange,
                    "liquidations",
                    canonical_symbol(row.get("symbol", self.symbol)),
                    "perpetual",
                    _timestamp(row.get("T") or payload.get("ts")),
...
class OKXWebSocketAdapter(WebSocketMarketDataAdapter):
        ...
        if channel == "trades":
            return [
                WebSocketEvent(
                    self.exchange,
                    "trades",
                    canonical_symbol(row["instId"]),
                    self.market_type,
                    _timestamp(row.get("ts")),
                    str(row["tradeId"]),
                    float(row["px"]),
                    float(row["sz"]),
                    str(row.get("side", "unknown")).lower(),
                    raw_payload=row,
                )
                for row in rows
            ]
        if channel in {"books", "books5", "bbo-tbt"}:
            return [
                WebSocketEvent(
                    ...
                    if row.get("seqId") is not None
                    else None,
                    previous_sequence_id=int(row["prevSeqId"])
                    if row.get("prevSeqId") is not None
                    else None,
                    update_type="snapshot" if channel == "books5" else "delta",
                    raw_payload=row,
                )
                for row in rows
            ]
    ...
    def _inst_id(self) -> str:
        ...
        return (
            ...
            if self.market_type == "perpetual"
            else f"{normalized[:-4]}-{normalized[-4:]}"
        )
