...
class ExternalSourceCollector:
    ...
    async def collect_deribit_options(self) -> list[Observation]:
        ...
        for item in payload["result"]:
            ...
            source_ts = (
                timestamp_from_ms(item["creation_timestamp"])
                if item.get("creation_timestamp")
                else None
            )
            for metric, field in (
                ("options_open_interest", "open_interest"),
    ...
    async def collect_fred_series(self, series_id: str) -> list[Observation]:
        ...
        if not api_key:
            raise RuntimeError(
                "FRED_API_KEY is required for FRED data and was not configured"
            )
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            response = await client.get(
        ...
        for item in payload.get("observations", []):
            if item.get("value") in (None, "."):
                continue
            timestamp = datetime.strptime(item["date"], "%Y-%m-%d").replace(
                tzinfo=timezone.utc
            )
            observations.append(
                Observation(
                    ...
                    "macro_value",
                    float(item["value"]),
                    metadata={"observation_date": item["date"], "series_id": series_id},
